  ...
  <form class="form" data-textarea="form-field-<?= $fieldname ?>" data-autosubmit="false" data-kirbytext="true">
    ...
    <div class="pages">
      <div class="rootpages">
        ...
        if (!function_exists("listpages")) {
          function listpages($subpages) {
            foreach ($subpages as $subpage) {
              ...
              echo('<span class="link smallbox active smalllink" data-code="'. $subpage->projectcode() .'" data-link="' . $subpage->uri() . '"><i class="icon fa fa-link"></i></span>');
        ...
        listpages($subpages = site()->children()->filterBy("intendedTemplate", "projects"));
        ?>
      </div>
    </div>
